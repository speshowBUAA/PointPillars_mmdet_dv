...
class PointPillars {
 private:
    ...
    // int kNumGatherPointFeature = 10;      // mmdet3d hv 是10
    int kNumGatherPointFeature = 9;      // mmdet3d dv 是9
    int kGridXSize;
    int kGridYSize;
    int kGridZSize;
    int kRpnInputSize;
    ...
    int* valid_num_points;

    int* host_pillar_count_;
    int* dev_ith_to_pointidx_;

    float* dev_pfe_gather_feature_;
    float* dev_pillar_pfn_feature_scatter_;
    int* dev_pillar_coors_x_;
    int* dev_pillar_coors_y_;
    int* dev_points_coors_;
    void* pfe_buffers_[2];
    void* rpn_buffers_[4];
    ...
    nvinfer1::Dims mPfeInputDims;  //!< The dimensions of the input of the MNIST model.
    nvinfer1::Dims mPfeOutputDims; //!< The dimensions of the output of the MNIST model.
    ...
    void OnnxToEngine(const std::string& model_file,
                      nvinfer1::ICudaEngine** engine_ptr);
    ...
    int setDynamicInputDimensions(const std::string &input_name,
                                  const int in_num_points,
